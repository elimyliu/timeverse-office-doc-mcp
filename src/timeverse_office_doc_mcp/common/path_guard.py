...
class PathGuard:
    ...
    相对路径统一锚定到 ServerConfig.BASE_DIR 解析，
    保证 MCP 客户端传入相对路径时结果可预测（不依赖进程 cwd）。
    """
    ...
    def __init__(
        ...
        self.base_dir = Path(base_dir or ServerConfig.BASE_DIR).resolve()

    def _resolve(self, file_path: str) -> Path:
        """解析路径：相对路径锚定到 base_dir，绝对路径直接 resolve。"""
        p = Path(file_path)
        if p.is_absolute():
            return p.resolve()
    ...
    def validate_path(self, file_path: str, operation: str = "read") -> str:
        ...
        检查项：
        1. 路径规范化（resolve .. 和符号链接；相对路径锚定到 BASE_DIR）
        ...
        if not self._is_allowed(resolved):
            raise ToolError(
                f"路径 '{file_path}'（解析为 '{resolved}'）不在允许的目录范围内"
            )
    ...
    def validate_directory(self, dir_path: str) -> str:
        ...
        if not self._is_allowed(resolved):
            raise ToolError(
                f"目录 '{dir_path}'（解析为 '{resolved}'）不在允许的目录范围内"
            )
